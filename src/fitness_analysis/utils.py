...
def rolling_linear_rate(
    ...
    *,
    ...
    observations. The grid step is inferred from the median index spacing. Each window's
    slope is computed using only the non-NaN observations, with their actual time
    offsets within the window used as x-values. When no values are missing the
    computation reduces to a fixed-kernel dot product.

    Args:
        series: Regularly-gridded time-indexed values. NaN entries are treated as
            missing observations and excluded from each window's regression.
        window: Rolling window size in grid steps.
        min_periods: Minimum number of non-NaN observations required to produce a
            result; windows with fewer yield NaN.
        units: Output rate units (e.g. ``"W"`` for per week, ``"D"`` for per day).
        center: If True, the window is centered on each observation. If False, the
            window is trailing (causal).

    Returns:
        Rolling slope aligned to the input index.
    """
    # Infer grid step from median index spacing to support any regular grid
    step_us = np.median(np.diff(np.array(series.index).astype("datetime64[us]").astype(np.float64)))
    units_per_step = (np.timedelta64(1, units) / np.timedelta64(1, "us")) / step_us
...
def infer_timezone(records: pd.DataFrame) -> str | None:
    ...
    Finds the first valid latitude and longitude in the data and calculates the timezone
    for that GPS location.

    Args:
        records: Activity data to process.

    Returns:
        Timezone name, or None if no valid latitude/longitude is available or no
        timezone match is found.
    """
...
def identify_inactive_periods(
    ...
    Returns a boolean mask identifying periods where the velocity of ``series`` (in
    units per second) remains below ``activity_threshold`` for at least
    ``min_duration``.

    Args:
        series: Time-indexed values.
        activity_threshold: Velocity, in units per second, below which values of
            ``series`` are considered inactive.
        min_duration: Minimum inactivity span to flag.

    Returns:
        Time-indexed boolean mask.
    """
    # Velocity in units/second between consecutive samples; first sample has no
...
def power_curve_windows(max_s: int, density: int) -> np.ndarray:
    ...
    Returns:
        Approximately logarithmically-spaced 1-D array of window sizes in seconds, from
        1 to ``max_s``, rounded to integers.
    """
    n = round(density * np.log10(max_s))
    return pd.unique(np.round(np.logspace(0, np.log10(max_s), n)).astype(int))


def compute_power_curve(power_series: pd.Series, windows: np.ndarray) -> np.ndarray | None:
    """Calculate max mean power (W) for each window size.

    Args:
        power_series: Power values with a DatetimeIndex. NaN entries are dropped before
            resampling.
        windows: Integer window sizes in seconds, as from ``power_curve_windows``.

    Returns:
        1-D array of max mean power (W) aligned to ``windows``, or ``None`` if there is
        no usable power data.
    """
    # Resample to a regular 1-second grid, treating gaps as coasting at 0 W
    power_arr = power_series.dropna().resample("s").mean().fillna(0.0).to_numpy(dtype=np.float64)
    if len(power_arr) == 0:
        return None

    # Re-use a single cumsum for each window computation: cumsum[n+window] - cumsum[n] =
    # window sum at n, dividing max by window gives best mean power
...
def pelt_segments(
    ...
    Scales ``signal`` before fitting so that all features contribute equally regardless
    of their units or variance.

    Args:
        signal: ``(N, D)`` DataFrame of features. Rows with any NaN are dropped before
            fitting.
        penalty: PELT penalty. Higher values produce fewer segments.
        min_size: Minimum observations per segment.

    Returns:
        One row per detected span with ``start`` and ``end`` columns. Empty if the
        signal has zero rows.
    """
    ...
    breakpoints = rpt.Pelt(model="l2", min_size=min_size, jump=1).fit(scaled).predict(pen=penalty)

    starts = [index[0]] + [index[bp] for bp in breakpoints[:-1]]
    ends = [index[bp - 1] for bp in breakpoints[:-1]] + [index[-1]]
    return pd.DataFrame({"start": starts, "end": ends}, index=range(len(starts)))
